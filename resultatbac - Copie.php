...
<div id="toolbar" align="right"><form action="accueil.php?bac" method="get"><input name="bac" type="hidden" value="" />Rechercher par le nom : <input name="nom" type="text" size="15" value="<?php
if(isset($_GET['nom'])){ echo $_GET['nom']; }  ?>" /> <input name="Recherche" type="submit" value="Rechercher" />|  <a href="?bac&ajoutlot" class="easyui-linkbutton" iconCls="icon-add" plain="true">Importer un fichier csv</a>  |<a href="" class="easyui-linkbutton" iconCls="icon-add" plain="true">Nouveau</a> </form>   
</div>  
<hr/>
...
if(isset($_GET['nom']) && $_GET['nom']!=""){
	$liste=selTableDataWhereLike("resultatbac","Nom",$_GET['nom']);
	$count=count($liste);
}else{
	$liste=selTableDataDescLimit("resultatbac","session","300");
	$count=300;
}
