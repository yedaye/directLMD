...
//			alert(type+"/"+num_table+"/"+session+"/"+ecole+"/"+annee_etude+"/"+mode+"/"+nationalite+"/"+filiere);
	...
	function ischecked(madiv,credit,montant_ecu){
		var montant;
...
<table width="75%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td width="50%">
	<p class="titre_form">Choisir votre mode d'accès à l'UAK</p>
...
//	  print_r($verdict);
	  if($verdict[0]['result_semestre_1']=="Refuse" || $verdict[0]['result_semestre_1']=="Chevauche"){
		//gestion des ecu ancien et nouvelle version
		if($an_precedent2=="2013-2014"){
			$ue=selTableMultiAnswerAsc("table_ue","code_ecole",$verdict[0]['filiere'],"designation");
		}else{
			$ue=selTableMultiAnswerAsc("table_ue_new","code_ecole",$verdict[0]['filiere'],"designation");	
		}
		$param=selTableDataWhere("param","anne_academique",$an_precedent);
		//print_r($ue);
		$a=0;
		for($i=0;$i<count($ue);$i++){
			$liste_ue="<tr><td colspan='3' bgcolor='#33DDEE'><b>UE : ".utf8_encode($ue[$i]['designation'])."</b></td></tr>";
			if($an_precedent2=="2013-2014"){
				$ecu=selTableMultiAnswerAsc("table_ecu","code_ue",$ue[$i]['code_ue'],"designation");
			}else{
				$ecu=selTableMultiAnswerAsc("table_ecu_new","code_ue",$ue[$i]['code_ue'],"designation");
			}
			$total=0;
			$test_ue="oui";
			$liste_ecu="";
...
//			echo $montant_ecu2;
  ...
  <td colspan="3" align="center" bgcolor="#FBEC88">Liste des ECU à reprendre pour l'année <b><?php echo $an_precedent2; ?></b> (ancienne note)</td>
    ...
    <td align="left" style="font-size:24px; color:#F00;" bgcolor="#FBEC88"><div align="left" id='total'><?php echo $montant1; ?></div></td>
...
//			echo $montant_ecu2;
  ...
  <td colspan="3" align="center" bgcolor="#FBEC88"><hr><br/>Liste des ECU à reprendre pour l'année <b><?php echo $an_precedent; ?></b> (ancienne note)</td>
    ...
    <tr>
    <td align="right" style="font-size:24px; color:#F00;" bgcolor="#FBEC88">MONTANT DES REPRISES : FF = </td>
    <td align="left" style="font-size:24px; color:#F00;" bgcolor="#FBEC88"><div align="left" id='total'><?php echo $montant2; ?></div></td>
    <td align="left" style="font-size:24px; color:#F00;" bgcolor="#FBEC88">FCFA  <?php  if($verdict[0]['result_semestre_1']=="Refuse"){ echo "/ FI=15000 FCFA"; } ?></td>
    </tr>
<?php
	  }   
