...
if(isset($_POST['lot'])){
		...
		while(($data=fgetcsv($handle,1000,";"))!==FALSE){
			$nom=mysql_real_escape_string($data[0]);
			$prenoms=mysql_real_escape_string($data[1]);
			$date_naissance=mysql_real_escape_string($data[2]);
			$lieu_naissance=mysql_real_escape_string($data[3]);
			$nationalite=mysql_real_escape_string($data[4]);
			$annee_auto=mysql_real_escape_string($data[5]);
			$observ_auto=mysql_real_escape_string($data[6]);
			$num_bac=mysql_real_escape_string($data[7]);		
			$session_bac=mysql_real_escape_string($data[8]);
			$matricule=mysql_real_escape_string($data[9]);
			$filiere=mysql_real_escape_string($data[10]);	
			$mode=mysql_real_escape_string($data[11]);	
			$ff=mysql_real_escape_string($data[12]);	
			$fi=mysql_real_escape_string($data[13]);	
			$type_auto=mysql_real_escape_string($data[14]);	
			$memo1=mysql_real_escape_string($data[15]);	
			$memo2=mysql_real_escape_string($data[16]);	
			$valide=mysql_real_escape_string($data[17]);
			$annee_inscrit=mysql_real_escape_string($data[18]);	
            
            
            
			$num_auto=num_autorisation($anneeEtude,$type_auto);
	...
	$valeur=array($num_auto,$nom,$prenoms,convertdatefrancais($date_naissance),$lieu_naissance,$nationalite,$annee_auto,$annee_inscrit,$observ_auto,$num_bac,$session_bac,$matricule,$filiere,$mode,$ff,$fi,$type_auto,$memo1,$memo2,$valide);
			insTable("autorisation",$champ,$valeur);
		}
...
if(isset($_POST['nom'])){
	...
	$num_auto=num_autorisation($anneeEtude,$_POST['type_auto']);
	$champ=array('code_auto','nom','prenoms','date_naissance','lieu_naissance','Nationalite','annee_auto','annee_inscrit','observ_auto','num_bac','session_bac','matricule','filiere','mode','FF','FI','type_auto','memo1','memo2','valide');
...
if(isset($_POST['modif'])){
	...
	updTable("autorisation",$champ,$valeur,"id",$_POST['modif']);
	echo "<script language='Javascript'>
		<!--
...
<tbody>
<?php
$liste=selTableDataDesc("autorisation","id");
...
?>
  ...
  <td><?php if($liste[$i]['valide']=='NON' || $liste[$i]['valide']==''){ ?><img onclick="valide(<?php echo $liste[$i]['id']; ?>,'OUI')" src="../img/valide.png" width="24" height="24" /><?php }else{ ?><img onclick="valide(<?php echo $liste[$i]['id']; ?>,'NON')" src="../img/cancel.png" width="24" height="24" /> <?php } ?></td>
    ...
    <td><?php
	$type=selTableDataWhere("type","id",$liste[$i]['type_auto']);
	 echo $type['name']; ?></td>
          ...
          <tr>
            ...
            <select name="nationalite" id="nationalite" class="easyui-combobox">
                ...
                <?php
                    ...
                    foreach ($pays as $py) {
                    	echo "<option value='".$py['cod_pays']."'>".utf8_encode(strtolower($py['lib_nation']))."</option>";
                    }
                ?>
            </select>
                   </td>
...
$modifcation=selTableDataWhere("autorisation","id",$_GET['modif']);
?>
