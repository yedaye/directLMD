...
$filiere = mysql_query($query_filiere) or die(mysql_error());
$row_filiere = mysql_fetch_assoc($filiere);
...
?> ><?php echo utf8_encode($row_filiere['libelle']); ?></option>
      	                <?php
			}
} while ($row_filiere = mysql_fetch_assoc($filiere));
  $rows = mysql_num_rows($filiere);
  if($rows > 0) {
      mysql_data_seek($filiere, 0);
	  $row_filiere = mysql_fetch_assoc($filiere);
  }
?>
    	                </select></td>
