...
<hr/>
<?php
 $etudiant=selTableData2Fields("inscription","filiere",$_POST['filiere'],"annee_academique",$_POST['annee']);
 //print_r($etudiant);
 if(count($etudiant)>0){
	$promotion=seltableDataWhere('student','matricule',$etudiant[0]['matricule']);
	$promotion=$promotion['promotion'];
 ?>
<br />
<table width='90%' align='center' class='timecard'>
<?php 
	$ecu=requete("SELECT table_ecu_new.code_ecu,table_ecu_new.designation FROM table_ecu_new, table_ue_new WHERE table_ue_new.code_ue=table_ecu_new.code_ue AND code_ecole='".$_POST['filiere']."' AND table_ue_new.promotion='".$promotion."' order by table_ecu_new.code_ecu ASC");
	//print_r($ecu); 
?>
	<tr><th> </th>
	<?php  
	...
	<?php 
		...
		for($i=0;$i<count($etudiant);$i++){
			...
			$note=requete("SELECT * FROM `result_ecu` WHERE matricule='".$etudiant[$i]['matricule']."' ORDER BY `code_ecu` ASC");
...
<div align="center" id="p3" class="easyui-panel" title="Ajout à partir d'un fichier excel CSV pour les mises à jour des redoublants" style="width:350px;height:200px;padding:10px;"
        data-options="iconCls:'icon-save',maximizable:true">
<form enctype="multipart/form-data" action="" method="post">
...
if(isset($_POST['lot'])){
		...
		while(($data=fgetcsv($handle,1000,";"))!==FALSE){
			...
			$code_ecu=mysql_real_escape_string($data[1]);
			$annee_academique=mysql_real_escape_string($data[2]);		
			$moyenne1=mysql_real_escape_string($data[4]);		
			...
			if($matricule!=$matricule1){
				...
				$sql="INSERT INTO verdict (matricule,filiere,annee_academique,result_semestre_1,result_semestre_2,moyenne,observation) VALUES ('".$matricule."','".$filiere."','".$annee_academique."','".$verdict."','".$verdict."','".$moyenne_gene."','')";
...
//COde pour la mise à jour des ecus repris
if(isset($_POST['lot2'])){
		//process the csv file
		...
		while(($data=fgetcsv($handle,1000,";"))!==FALSE){
			...
			$code_ecu=mysql_real_escape_string($data[1]);
			$annee_academique=mysql_real_escape_string($data[2]);		
			$moyenne1=mysql_real_escape_string($data[3]);		
			$moyenne1=explode(",",$moyenne1);
			$moyenne=$moyenne1[0].".".$moyenne1[1];
			$sql="UPDATE result_ecu SET moyenne='".$moyenne."' WHERE matricule='".$matricule."' AND code_ecu='".$code_ecu."' AND annee_acad='".$annee_academique."'";
			mysql_query($sql) or die(mysql_error());
		}
		header("Location:?liste_result_ecu");
		mysql_close();
