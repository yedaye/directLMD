...
if(isset($_POST['code'])){
	...
	$controle=selTableDataCount2("table_ecu_new","code_ecu",$_POST['code'],"promotion",$_POST['promo'],$pdo);
	if($controle==0){
		...
		insTable("table_ecu_new",$champ,$valeur,$pdo);
...
if(isset($_POST['modif'])){
	...
	updTableWhereArray("table_ecu_new",$champ,$valeur,$ArWherField,$ArWherVal,$pdo);
	echo "<script language='Javascript'>
		<!--
...
///ajout des UE par un fichier csv
// ajout par lot 
if(isset($_POST['lot'])){
    // print_r($_POST['lot']);
     ...
     while(($data=fgetcsv($handle,1000,";"))!==FALSE){
        ...
        $champ=array('code_ecu','designation','credit','code_ue','promotion','semestre');
        $valeur=array($code_ecu,$designation,$credit,$code_ue,$promotion,$semestre);
        insTable("table_ecu_new",$champ,$valeur,$pdo);
     }
     echo "<script language='Javascript'>
     <!--
...
<a href="?ecu_new&ajoutlot" class="easyui-linkbutton" iconCls="icon-add" plain="true">Importer un fichier csv</a>  | <a href="?ecu_new&ajout" class="easyui-linkbutton" iconCls="icon-add" plain="true">Nouveau</a>  
     <form method='GET'>  <select required="true" name="promotion" id="promotion" onChange='submit();'>
                    ...
                    <option value='P5' <?php if($modification[0]['promotion']=='P5'){ echo "selected=\"selected\""; } ?>>Promo5 (2017-2018)</option>
					<option value='P6' <?php if($modification[0]['promotion']=='P6'){ echo "selected=\"selected\""; } ?>>Promo6 (2018-2019)</option>
...
<?php
if(!isset($_GET['promotion'])){
	$liste=selTableMultiAnswer("table_ecu_new","promotion","P1",$pdo);
}else{
	$liste=selTableMultiAnswer("table_ecu_new","promotion",$_GET['promotion'],$pdo);
}
          ...
          <tr>
            ...
            <td>
                <select required="true" name="promo" id="promo">
                    ...
                    <option value='P5'>Promo5 (2017-2018)</option>
					<option value='P6'>Promo6 (2018-2019)</option>
                </select>
            </td>
          </tr>
		  <tr>
            ...
            <td>
                <select required="true" name="code_ue" id="code_ue">
                    <?php
                    $ufr=selTableDataUnique3Fields("table_ue_new","code_ue","designation","promotion",$pdo);
...
$modification=selTableData2Fields("table_ecu_new","code_ecu",$_GET['modif'],'promotion',$_GET['promo'],$pdo);
?>
<div align="center" id='retour' style="display:none"> <a href="ecu_new.php" class="easyui-linkbutton">RETOUR</a></div>
