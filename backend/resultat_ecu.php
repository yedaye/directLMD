...
//exit;
	...
	for($i=0;$i<count($_POST['ecu']);$i++){
		$monecu=$_POST['ecu'][$i];
		$champ=array('moyenne');
		$valeur=array($_POST[$monecu]);
		$ArWherField=array("matricule","code_ecu","annee_acad");
		$ArWherVal=array($_POST['matricule2'],$monecu,$_POST['annee_acad']);
...
if(isset($_POST['matricule'])){  
	...
	$inscription=selTableMultiAnswer("inscription","matricule",$_POST['matricule'],$pdo);
	$result_ecu=selTableData2Fields("result_ecu","matricule",$_POST['matricule'],"annee_acad",$_POST['annee'],$pdo);
	//if(count($result_ecu)>0){	$result_ecu=$result_ecu[0]; }
	//print_r($result_ecu);
	$inscrit_encours=selTableData2Fields("inscription","matricule",$_POST['matricule'],"annee_academique",$_POST['annee'],$pdo);
	/*  print_r($etudiant);
	echo '<br/>';
	print_r($inscription);
	echo '<br/>';
      ...
      <td width="0" colspan="2" rowspan="2" align="center" valign="top"><table width="100%" border="0" align="left" cellpadding="0" cellspacing="2" class="texte_grand">
        ...
        <tr>
          <td bgcolor="">
            ...
            Année d'études : <?php 
//			print_r($inscrit_encours);
			 if(count($inscrit_encours)>0){echo $inscrit_encours['filiere'];} ?><br />
              <br />
      ...
      <td width="76%" align="center">Relevé de notes du matricule : <?php echo $etudiant['matricule']; ?><input name="matricule2" type="hidden" value="<?php echo $etudiant['matricule']; ?>" /><input name="annee_acad" type="hidden" value="<?php if(count($inscrit_encours)>0){echo   $inscrit_encours['annee_academique']; }  ?>" /></td>
    </tr>
    <tr>
 ...
 <?php
	...
	if(count($inscrit_encours)>0){
		$lafiliere=$inscrit_encours['filiere'];
		echo $lafiliere;
		$filbon=$lafiliere;
		$lafiliere=explode("-",$lafiliere);
		$lafiliere=$lafiliere[1];
		/* if($lafiliere=='ESTCTPA' AND $_POST['annee']=='2016-2017'){
			$verdicte=selTableData2Fields('verdict','matricule',$inscription[$i]['matricule'],'annee_academique',$_POST['annee'],$pdo);
			//print_r($verdicte);
			if(count($verdicte)>0){
				$verdicte=$verdicte[0];
		...
		//echo '!!!'.$filbon;
		$ue=selTableData2FieldsAsc("table_ue_new","code_ecole",$filbon,"promotion",$etudiant['promotion'],"designation",$pdo);
		//print_r($ue);
		$param=selTableDataWhere("param","annee_academique",$inscrit_encours['annee_academique'],$pdo);
		//print_r($param);
	}
...
for($i=0;$i<count($ue);$i++){
 ?>
  <tr bgcolor="#D0DBF9">
    <td bgcolor="#D0DBF9"><b><?php echo $ue[$i]['designation']."(".$ue[$i]['code_ue'].")"; ?></b></td>
 ...
 <?php 
	...
	?>
    <tr bgcolor="#FFFFCC">
    <td bgcolor="#FFFFCC"><?php echo $ecu[$a]['designation']."(".$ecu[$a]['code_ecu'].")"; ?><input name="ecu[]" type="hidden" id="ecu[]" size="10" value="<?php echo $ecu[$a]['code_ecu']; ?>"/></td>
