...
if(isset($_POST['ajouter'])){
	...
	$controle=selTableDataWhereArray("penalite",$array_field,$array_values,$pdo);
	if(count($controle)==0){
		...
		insTable("penalite",$champ,$valeur,$pdo);
	...
	$inscription=selTableData2Fields("inscription","matricule",$etudiant['matricule'],"annee_academique",$_POST['annee_acad'],$pdo);
...
if(isset($_POST['modif'])){
	...
	updTable("penalite",$champ,$valeur,"id",$_POST['modif'],$pdo);
	echo "<script language='Javascript'>
		<!--
	...
	// génération du XML
	$etudiant=selTableDataWhere("student","matricule",$_POST['matricule2'],$pdo);
	$inscription=selTableData2Fields("inscription","matricule",$etudiant['matricule'],"annee_academique",$_POST['annee_acad2'],$pdo);
	//$montantdu=$inscription[0]['FF']+$inscription[0]['FI']+$inscription[0]['montant_reprise'];
...
<tbody>
<?php
$liste=selTableDataDesc("penalite","id",$pdo);
...
?>
  <tr valign="top">
    ...
    <td><?php echo utf8_decode($liste[$i]['observation']); ?></td>
...
<!-- formulaire d'ajout  -->
<div align="center" id="p" class="easyui-panel" title="Ajout d'une nouvelle penalite" style="width:700px;height:400px;padding:10px;"
...
$modification=selTableDataWhere("penalite","id",$_GET['modif'],$pdo);
?>
       ...
       <table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr>
            ...
            <td><select name="annee_acad2" id="annee_acad2">
              <?php
			$ufr=selTableData("annee_academique","",$pdo);
			for($i=0;$i<count($ufr);$i++){
				$select="";
