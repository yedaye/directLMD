...
if(isset($_POST['code'])){
	...
	$controle=selTableDataCount("table_ecu","code_ecu",$_POST['code']);
	if($controle==0){
		$champ=array('code_ue','designation','credit','code_ecu');
		$valeur=array($_POST['code_ue'],$_POST['libelle'],$_POST['credit'],$_POST['code']);
		insTable("table_ecu",$champ,$valeur);
		echo "<script language='Javascript'>
		<!--
		document.location.replace('?ecu&ajoutOK');
		// -->
		</script>";
	}else{
		echo "<script language='Javascript'>
		<!--
		document.location.replace('?ecu&ajoutDJA');
		// -->
		</script>";
	}
}
...
if(isset($_POST['modif'])){
	$champ=array('code_ecu','designation','credit','code_ue');
	$valeur=array($_POST['code2'],$_POST['libelle2'],$_POST['credit2'],$_POST['code_ue2']);
	updTable("table_ecu",$champ,$valeur,"code_ecu",$_POST['modif']);
	echo "<script language='Javascript'>
		<!--
		document.location.replace('?ecu&modifOK');
		// -->
		</script>";
}
...
function suppression(val){
	 $.messager.confirm('Confirm','Voulez vous supprimer cette ECU : '+val,function(r){  
		if (r){  
			$.post('../js/xphp/sup/sup_ecu.php',{code:val},function(data){  
				if(data==1){
					document.location.href="?ecu&supOK";
				}
			});  
		}  
	});  	
...
<div id="toolbar" align="right">  
     <a href="?ecu&ajout" class="easyui-linkbutton" iconCls="icon-add" plain="true">Nouveau</a>    
</div>  
<hr/>
...
<tbody>
<?php
$liste=selTableData("table_ecu","code_ecu");
...
?>
  <tr valign="top">
    ...
    <td>  
<a href="?ecu&modif=<?php echo $liste[$i]['code_ecu']; ?>" class="easyui-linkbutton" iconCls="icon-edit" plain="true">Modifier  </a>
<a onclick="suppression('<?php echo $liste[$i]['code_ecu']; ?>')" class="easyui-linkbutton" iconCls="icon-remove" plain="true">Supprimer </a>
    </td>
  </tr>
<?php
}
...
<?php } ?>
<?php if(!isset($_GET['modif']) && isset($_GET['ajout'])){ ?>
<div align="center" id='retour' style="display:none"> <a href="ecu.php" class="easyui-linkbutton">RETOUR</a></div>
<!-- formulaire d'ajout  -->
          ...
          <tr>
            ...
            <td>
                <select required="true" name="code_ue" id="code_ue">
                    <?php
                    $ufr=selTableData("table_ue","code_ue");
                    for($i=0;$i<count($ufr);$i++){
                        echo "<option value='".$ufr[$i]['code_ue']."'>".$ufr[$i]['designation']."</option>";
                    }
                    ?>
                </select>
            </td>
...
$modification=selTableDataWhere("table_ecu","code_ecu",$_GET['modif']);
?>
...
<div id="p" class="easyui-panel" title="Modification ECU" style="width:850px;height:400px;padding:10px;"
    ...
    <form id="fm2" method="post">  
        <table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr>
            ...
            <td>
                <select required="true" name="code_ue2" id="code_ue2">
                    ...
                    for($i=0;$i<count($ufr);$i++){
                        $selected="";
                        if($modification['code_ue']==$ufr[$i]['code_ue']){ $selected="selected=\"selected\""; }
                        echo "<option value='".$ufr[$i]['code_ue']."' ".$selected.">".$ufr[$i]['designation']."</option>";
                    }
                    ?>
                </select>
            </td>
          ...
          <tr>
            <td>Code ECU</td>
            <td><input name="code2" id="code2" class="easyui-validatebox" required="true" value="<?php echo $modification['code_ecu']; ?>"></td>
          </tr>
          <tr>
            <td>Libellé ECU</td>
