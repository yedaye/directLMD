...
?>
     Filtrer par Etablissement : <select name="ecoles" id="ecoles">
      <?php
			$ufr=selTableDataDesc("ecole_ufr","code_ecole", $pdo);
...
if(isset($_GET['annee2'])){
	...
	if(isset($_GET['ecoles']) && $_GET['ecoles']!=""){
		...
		}elseif ($_GET['ecoles']=='CAG1') {
			$where="AND (filiere.code LIKE 'LP1,2-%') ";	
		}elseif ($_GET['ecoles']=='CAG2') {
			$where="AND (filiere.code LIKE 'LP3,4-%') ";	
		}else{
 ...
 $qstr = "SELECT FF, count( * ) as nombre, ecole FROM `inscription`, filiere WHERE filiere.code=inscription.filiere AND controle = 'oui' AND annee_academique = '".$an."' ".$where."  AND FI!=15000 GROUP BY FF"; 
 ...
 //echo "SELECT FF, count( * ) as nombre, ecole FROM `inscription`, filiere WHERE filiere.code=inscription.filiere AND controle = 'oui' AND annee_academique = '".$an."' ".$where."  AND FI!=15000 GROUP BY FF";
	if($nombre>0){
		...
		echo "<span class='Style6'>Pas encore d'inscription validé</span>";
	}
?>
</td>
...
if(in_array("uak",$_SESSION['etablissement'])){
 ...
 $qstr2 = "SELECT ecole, SUM(FF) as FF, SUM(FI) as FI, SUM(`montant_reprise`) as reprise FROM `inscription`, filiere WHERE controle = 'oui' AND annee_academique='".$an."' AND filiere.code=inscription.filiere ".$where."  GROUP BY filiere.ecole ORDER BY filiere.ecole";
 
 //echo $qstr2;
 $stm2 = $pdo->query($qstr2);
 $nombre2 = $stm2->rowcount();	
 if($nombre2>0){
		echo "<table id='source2' width='95%' border='0' cellpadding='0' cellspacing='2' align='center'>
				...
				foreach($rows as $element) {	
					...
					$rsm3 = "SELECT SUM(montant) as penalite FROM inscription,`penalite` WHERE inscription.matricule=penalite.matricule AND inscription.annee_academique=penalite.annee_academique AND inscription.controle='oui' AND penalite.annee_academique='".$an."' AND penalite.etablissement='".$element['ecole']."'";
					$stm3 = $pdo->query($rsm3);
 					$isnull = $stm3->rowcount(); 
					if($isnull>0){
						$elements = $stm3->fetchAll(PDO::FETCH_ASSOC);
...
if(in_array("uak",$_SESSION['etablissement'])){
	...
	if($_GET['ecoles']=='CAG'){
	 ...
	 $qstr = "SELECT ecole, COUNT(*) as nombre, SUM(montant_reprise) as montant FROM `inscription`,filiere WHERE controle = 'oui' AND  filiere.code=inscription.filiere AND montant_reprise!=0 AND annee_academique='".$an."' ".$where." GROUP BY filiere.ecole"; 
