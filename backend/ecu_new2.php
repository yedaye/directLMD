...
function charge_ue(promo,dive,tipe){
	//alert(promo);
	//alert(dive);
	//alert(type);
	$('#'+dive).hide('slow');
	$.post("../js/xphp/charge_ue.php", {lep:promo, letype:tipe}, function(data){
		alert(data);
		if(data.length >0){
			$('#'.dive).html(data);
		}else{
			alert("Aucun Promo sélectionnée");	
		}
	});
	$('#'+dive).show('slow');
}
    ...
    <form id="fm" method="post">  
        <table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr>
            <td>Promotion</td>
            <td>
                <select required="true" name="promo" id="promo" onChange="charge_ue(this.value,'lesue','ajout');">
...
<div id="p" class="easyui-panel" title="Modification ECU" style="width:850px;height:400px;padding:10px;"
    ...
    <form id="fm2" method="post">  
		...
		<tr>
            <td>Promotion</td>
            <td>
                <select required="true" name="promo2" id="promo2" onChange="charge_ue(this.value,'lesue2','modif');">
