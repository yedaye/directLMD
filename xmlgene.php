...
if(isset($_GET['matricule']) || !isset($_GET['filieres'])){
	if($_GET['matricule']!=""){
		...
		$montant=$inscription[0]['FF']+$inscription[0]['FI']+$inscription[0]['montant_reprise'];
		$ecole=selTableDataWhere('filiere','code',$inscription[0]['filiere']);
		
		$xmlfile=genexml($_GET['matricule'],$etudiant['nom'],$etudiant['prenoms'],$etudiant['date_naissance'],$etudiant['lieu_naissance'],$etudiant['telephone'],$montant,$inscription[0]['annee_academique'],$inscription[0]['statut'],$ecole['ecole'],$inscription[0]['filiere'],$etudiant['nationalite'],"inscription");
		echo "fichier de ".$etudiant['matricule']." généré";
	}
	if($_GET['matricule']=="" && $_GET['filieres']!=""){
			...
			for($i=0;$i<count($liste);$i++){
				...
				$xmlfile=genexml($etudiant['matricule'],$etudiant['nom'],$etudiant['prenoms'],$etudiant['date_naissance'],$etudiant['lieu_naissance'],$etudiant['telephone'],$montant,$inscription[0]['annee_academique'],$inscription[0]['statut'],$ecole['ecole'],$inscription[0]['filiere'],$etudiant['Nationalite'],"inscription");
				var_dump($xmlfile);
				//echo "/////".$i."//////".$etudiant['matricule']."//".$etudiant['nom']."//".$etudiant['prenoms']."//".$etudiant['date_naissance']."//".$etudiant['lieu_naissance']."//".$etudiant['telephone']."//".$montant."//".$inscription[0]['annee_academique']."//".$inscription[0]['statut']."//".$ecole['ecole']."//".$inscription[0]['filiere']."//".$etudiant['Nationalite']."//"."inscription<br/>";
			}
