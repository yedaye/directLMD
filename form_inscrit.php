...
if($type=='bachelier' and !isset($_GET['modifier'])){
	...
	$etudiant=selTableData2Fields('resultatbac','NumTable',$_POST['numbac'],"session",$_POST['anneebac'],$pdo);
	//$etudiant=$etudiant[0];
	//print_r($etudiant);
...
if($type=='reinscription' and !isset($_GET['modifier'])){
	//recuperation des infos du bachelier
	$etudiant=selTableDataWhere('student','matricule',$_POST['matricule'],$pdo);
	//print_r($etudiant);
...
if($type=='autorisation' and !isset($_GET['modifier'])){
	//recuperation des infos du bachelier
	$auto=selTableData2Fields('autorisation','code_auto',$_POST['codeauto'],'annee_auto',$anneeEtude,$pdo);
	//print_r($etudiant);
	if(count($auto)>0){
        //print_r($auto);
		//$auto=$auto[0];
		...
		if($auto['matricule']!=""){
			...
			$etudiant=selTableDataWhere('student','matricule',$matricule,$pdo);
			//print_r($etudiant);
			$num_table=$etudiant['num_table'];
			$serie_bac=$etudiant['serie'];
			$session=$etudiant['session'];;
        ...
        echo "Pas d'autorisation avec ce code";
    }
...
<script type="text/javascript" language="javascript1.2">
	...
	function controles(){
		...
		if(mode!="" && ecole!="" && annee_etude!=""){
			//alert(montype+"/"+num_table+"/"+session+"/"+ecole+"/"+annee_etude+"/"+mode+"/"+nationalite+"/"+filiere);
...
$a=0;
if($type=='bachelier'){
	$verif=selTableData2Fields('student','num_table',$num_table,'session',$_POST['anneebac'],$pdo);
	if(isset($verif['matricule']) && $verif['matricule']!=""){
		//recuperation des données de l'étudiant
		$matricule=$verif['matricule'];
		$num_table=$verif['num_table'];
		$serie_bac=$verif['serie'];
		$session=$verif['session'];;
		$moyenne_bac=$etudiant['Moyenne'];	
		$nom=$verif['nom'];
		$dis_nom="readonly";
		$prenoms=$verif['prenoms'];
		$dis_prenom="readonly";
		$codsexe=$verif['sexe'];
		$dis_sexe="readonly";
		$date_nais=$verif['date_naissance'];
		$dis_date_nais="readonly";
		$lieu_nais=$verif['lieu_naissance'];
		$dis_lieu_nais="readonly";
		$codpays=$verif['pays_naissance'];
		$codnation=$verif['Nationalite'];
		$departement=$verif['departement'];
		$codesitufam=$verif['situ_fam'];
		$nombre_enfant=$verif['nombre_enfant'];
		$telephone=$verif['telephone'];
		$email=$verif['email'];
		$bp=$verif['adresse_postal'];
		//$promotion=$verif[0]['promotion'];
		$codannee_etude=$anneeEtude;
		
		//fin recuperation
		$inscrit=selTableData2Fields('inscription','matricule',$verif['matricule'],'annee_academique',$anneeEtude,$pdo);
		if(isset($inscrit['filiere']) && $inscrit['filiere']!=""){
			$msg=" <a class='titre_form' href='backend/fiche.php?matricule=".$matricule."&annee=".$anneeEtude."'>IMPRIMER MA FICHE DE PREINSCRIPTION</a><br/><strong>Vous êtes déja inscrit en ".$inscrit['filiere']." au cours de l'année académique ".$anneeEtude."</strong>";	
...
if($type=='reinscription'){
	$inscrit=selTableData2Fields('inscription','matricule',$matricule,'annee_academique',$anneeEtude,$pdo);
	if(isset($inscrit['filiere']) && $inscrit['filiere']!=""){
		$msg=" <a class='titre_form' href='backend/fiche.php?matricule=".$matricule."&annee=".$anneeEtude."'>IMPRIMER MA FICHE DE PREINSCRIPTION</a><br/><strong>Vous êtes déja inscrit en ".$inscrit['filiere']." au cours de l'année académique ".$anneeEtude."</strong>";	
		echo "<center>".$msg."</center><div align='center' id='retour'> <a href='inscription.php?reset' class='easyui-linkbutton'>RETOUR</a></div>";
	}else{
	...
	$inscritcontrole=selTableDataWhereArray('inscription',$arrayfield,$arrayvalue,$pdo);
...
if($type=='autorisation'){
	...
	$num=selTableDataCount('student','code_auto',$code_auto,$pdo);
	//print_r($num);
	if($num>0){
		$info=selTableDataWhere('student','code_auto',$code_auto,$pdo);
...
<table width="75%" border="0" align="center" cellpadding="0" cellspacing="0">
       ...
       <tr>
        ...
        <tr>
          ...
          <td valign="top"><label for="lieu_naissance"></label>
            <?php 
				if($lieu_nais!=""){ ?>
					<input name="lieu_naissance" id="lieu_naissance" size="5" value="<?php echo $lieu_nais; ?>" type="hidden"/>
			<?php 
					echo $lieu_nais;
				}else{ ?>
            		<input type="text" name="lieu_naissance" id="lieu_naissance" class="easyui-validatebox" data-options="required:true" value="<?php echo $lieu_nais; ?>" <?php echo $dis_lieu_nais; ?>/></td>
	        <?php } ?>
        </tr>
        <tr bgcolor="#D6D6D6">
          ...
          <td valign="top"><label for="pays_naissance"></label>
             <?php if($codpays!=""){ ?>
                        ...
                        <?php 
						$libpays=selTableDataWhere('pays','cod_pays',$codpays,$pdo);
						echo $libpays['lib_pays'];
						}else{ ?>
						...
						<?php
							...
							echo "<option value='".$py['cod_pays']."' ".$spy.">".strtolower($py['lib_pays'])."</option>";
							}
						?>
                        </select>
                        <?php } ?>
        ...
        <tr>
          ...
          <td valign="top"><label for="nationalite"></label>
            <?php if($codnation!=""){ ?>
                        ...
                        <?php 
							$libpays=selTableDataWhere('pays','cod_pays',$codnation,$pdo);
							echo $libpays['lib_nation'];
						}else{ 
						?>
        ...
        <tr bgcolor="#D6D6D6">
          ...
          <td valign="top"><label for="departement"></label>
			...
			if($departement!=""){ ?>
                        ...
                        <?php 
							$libpays=selTableDataWhere('departement','code_dept',$departement,$pdo);
							echo $libpays[0]['lib_dept'];
						}else{ 
						?>
			            <select name="departement" id="departement" class="easyui-combobox">
						<?php
							$pays = selTableData('departement','lib_dept',$pdo);
                            //print_r($pays);
                            echo "yann";
							$a=0;
							foreach ($pays as $py) {
							...
							echo "<option value='".$py['code_dept']."' ".$spy.">".strtolower($py['lib_dept'])."</option>";
							}
						?>
                        </select>
            <?php } ?></td>
    ...
    <td colspan="2"><p class="titre_form">Choississez l'école dans laquelle vous voulez vous inscrire :</p>
      <p>
        <input name="ecole_bon" id="ecole_bon" type="hidden" value=""/>
           <select name="ecole" id="ecole" class="easyui-combobox" style="width:600px" >
             <option value=""></option>
                <?php
                    $ecoles = selTableData('ecole_ufr',"",$pdo);
                    //echo "yann";
                    //print_r($ecoles);
                    $a=0;
                    foreach ($ecoles as $ecole) {
                        if($ecole['code_ecole']==$codannee_etude){ 
                            $spy="selected"; 								
                        }else{
                            $spy="";
                        }
                        
                        echo "<option value='".$ecole['code_ecole']."' ".$spy.">".utf8_encode(strtoupper("(".$ecole['code_ecole'].") =&gt; ".$ecole['lib_ecole']))."</option>";	
  ...
  <tr>
	...
	<p class="titre_form">Choisir votre mode d'accès à l'UNA</p>
            ...
            <option value=""></option>
            <?php
                $mode = selTableMultiAnswer('mode','actif',1,$pdo);
                $a=0;
                foreach ($mode as $mo) {
                if($mo['code']==$codmode){ 
  ...
  if($_GET['type']=="reinscription"){ 
	...
	//volet concernant l'année en cours
		$affichage="";
		$verdict=selTableData2Fields("verdict","matricule",$matricule,"annee_academique",$an_precedent,$pdo);
		$result_ecu=selTableData2Fields("result_ecu","matricule",$matricule,"annee_acad",$an_precedent,$pdo);
		$letest=explode("-",$verdict['filiere']);
		$filiere1=$verdict['filiere'];
		//print_r($result_ecu);
		//print_r($verdict);
		//gestion des ecu ancien et nouvelle version
		
		$ue=selTableData2FieldsAsc("table_ue_new","code_ecole",$verdict['filiere'],"promotion",$etudiant['promotion'],"designation",$pdo);
		$param=selTableDataWhere("param","annee_academique",$anneeEtude,$pdo);
		//print_r($ue);
		//print_r($param);
		$a=0;
		for($i=0;$i<count($ue);$i++){
			$liste_ue="<tr><td colspan='3' bgcolor='#33DDEE'><b>UE : ".$ue[$i]['designation']."</b></td></tr>";
			$ecu=selTableData2FieldsAsc("table_ecu_new","code_ue",$ue[$i]['code_ue'],"promotion",$etudiant['promotion'],"designation",$pdo);
			$total=0;
			$test_ue="oui";
			$liste_ecu="";
			$liste_reserve="";
			...
			for($a=0;$a<count($ecu);$a++){
				...
				for($o=0;$o<count($result_ecu);$o++){
					...
					if($result_ecu[$o]['code_ecu']==$ecu[$a]['code_ecu']){ 
						...
						if($result_ecu[$o]['moyenne']<$param['moyenne_ecu_mini']){
							...
							$liste_ecu.="<tr align='left'><td>".$ecu[$a]['designation']." (".$result_ecu[$o]['moyenne']."/20)</td><td>".$ecu[$a]['credit']."</td><td align='left'><input name='ECU[]' type='checkbox' value='".$ecu[$a]['code_ecu']."' onchange=\"nodischecked('".$ecu[$a]['code_ecu']."')\" id='".$ecu[$a]['code_ecu']."' checked /><div align='right'> ".round($param['montant_ecu']*$ecu[$a]['credit'])." FCFA</div></td></tr>";
						}else{
							$liste_reserve.="<tr align='left'><td>".$ecu[$a]['designation']." (".$result_ecu[$o]['moyenne']."/20)</td><td>".$ecu[$a]['credit']."</td><td align='left'><input name='ECU[]' type='checkbox' value='".$ecu[$a]['code_ecu']."' onchange=\"ischecked('".$ecu[$a]['code_ecu']."','".$ecu[$a]['credit']."','".$param['montant_ecu']."','total1')\" id='".$ecu[$a]['code_ecu']."'/> <div id='montant_".$ecu[$a]['code_ecu']."' align='right'></div></td></tr>";	
						}
						$k++;						
					}
				}
				if($k==0){
					$montant_ecu=$montant_ecu+round($param['montant_ecu']*$ecu[$a]['credit']);
					$montant_ecu2=$montant_ecu2+$montant_ecu;
					$liste_ecu.="<tr align='left'><td>".$ecu[$a]['designation']." (0/20)</td><td>".$ecu[$a]['credit']."</td><td align='left'><input name='ECU[]' type='checkbox' value='".$ecu[$a]['code_ecu']."' onchange=\"nodischecked('".$ecu[$a]['code_ecu']."','total1')\" id='".$ecu[$a]['code_ecu']."' checked /><div align='right'> ".round($param['montant_ecu']*$ecu[$a]['credit'])." FCFA</div></td></tr>";
				}
			}
...
//			echo $montant_ecu2;
  ...
  <td colspan="3" align="center" bgcolor="#FBEC88"><hr><br/>Liste des ECU à reprendre pour l'année <b><?php echo $an_precedent; ?></b> (ancienne note)</td>
    ...
    <td align="left" style="font-size:24px; color:#F00;" bgcolor="#FBEC88">FCFA  <?php  if($verdict['result_semestre_1']=="Refuse"){ echo "/ FI=15000 FCFA"; } ?></td>
    </tr>
...
<?php
	...
	/////////////////////////////////////////////////////////////////////////////volet concernant deux ans en arrière 
	  $verdict=selTableData2Fields("verdict","matricule",$matricule,"annee_academique",$an_precedent2,$pdo);
	  $result_ecu=selTableData2Fields("result_ecu","matricule",$matricule,"annee_acad",$an_precedent2,$pdo);
	  $etab_controle=explode("-",$verdict['filiere']);
	  $filiere2=$verdict['filiere'];
	  //print_r($etab_controle);
	  if(($verdict['result_semestre_1']=="Refuse" || $verdict['result_semestre_1']=="Chevauche") && $filiere2!=$filiere1){
		 // echo "test";
		  //exit;
		//gestion des ecu ancien et nouvelle version
		$ue=selTableData2FieldsAsc("table_ue_new","code_ecole",$verdict['filiere'],"promotion",$etudiant['promotion'],"designation",$pdo);
		//print_r($ue);
		$param=selTableDataWhere("param","annee_academique",$an_precedent2,$pdo);
		//print_r($ue);
		$a=0;
		for($i=0;$i<count($ue);$i++){
...
<?php   
	...
	//////////////////////////////////////////////////////////////////////////////////////volet concernant trois ans en arrière 
	  $verdict=selTableData2Fields("verdict","matricule",$matricule,"annee_academique",$an_precedent3,$pdo);
	 // print_r($verdict);
	  $result_ecu=selTableData2Fields("result_ecu","matricule",$matricule,"annee_acad",$an_precedent3,$pdo);
	  $etab_controle=explode("-",$verdict['filiere']);
	  $filiere3=$verdict['filiere'];
	  //print_r($etab_controle);
	  if(($verdict['result_semestre_1']=="Refuse" || $verdict['result_semestre_1']=="Chevauche") && $etab_controle[1]!='ESTCTPA' && $filiere3!=$filiere2){
		 // echo "test";
		  //exit;
		//gestion des ecu ancien et nouvelle version
		$ue=selTableData2FieldsAsc("table_ue_new","code_ecole",$verdict['filiere'],"promotion",$etudiant['promotion'],"designation",$pdo);
		//print_r($ue);
		$param=selTableDataWhere("param","annee_academique",$an_precedent,$pdo);
		//print_r($ue);
		$a=0;
		for($i=0;$i<count($ue);$i++){
			$liste_ue="<tr><td colspan='3' bgcolor='#33DDEE'><b>UE : ".utf8_encode($ue[$i]['designation'])."</b></td></tr>";
