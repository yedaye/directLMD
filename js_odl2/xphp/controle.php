...
//			print_r($statut);
			...
			if(count($resultat)>0){
				...
				$test=selTableData2Fields("mapping","filiere",$resultat[0]['filiere'],'verdict',$resultat[0]['result_semestre_2'],$pdo);
				//echo $lafiliere;
				...
				if(count($test)>0){
					...
					if($test[0]['fil_auto']==$lafiliere){
						if($resultat[0]['result_semestre_2']=="Refuse"){
						...
						if( $resultat[0]['result_semestre_2']=="Chevauche"){
							//if($lecole!='STA' && !in_array($lematricule,$liste_matricule)){
							//	$val=lemontant($lafiliere,$lanationalite,$lemode);
							//	echo $val;
							//	echo "<script>ouvrir('reprise');document.getElementById('valide').value='oui';</script>";	
						...
						if( $resultat[0]['result_semestre_2']=="Admis"){
							$val=lemontant($lafiliere,$lanationalite,$lemode);
							echo $val;
						}
						if(isset($resultat2) && count($resultat2)>0){
							if($resultat2[0]['result_semestre_2']=='Refuse' || $resultat2[0]['result_semestre_2']=='Chevauche'){
								echo "<script>ouvrir('reprise');document.getElementById('valide').value='oui';</script>";	
							//echo $val;
							}
						}
